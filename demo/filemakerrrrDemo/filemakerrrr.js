...
export class Filemakerrrr {
    ...
    constructor({ downloadName = 'miFile', alwaysZip = false, verbose = false, talkToMeCallbak = console.log, lang = 'es', } = {}, stringForFastZip) {
        this.#settings = {
    ...
    async zip() {
        ...
        const { charsMap, charsUnicode } = await stringChecker(this.#zipInfo.in);
        const { should, rate } = zipForecast(this.#zipInfo.in.length, charsMap.size, charsUnicode);
        this.#stats = {
            ...this.#stats,
            action: 'zip',
            chars: charsMap.size,
    ...
    #talkToYou([process, key, args], always = false) {
        ...
        const finalOutput = typeof baseOutput === 'function' && args
            ? baseOutput(args)
            : baseOutput;
